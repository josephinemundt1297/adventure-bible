...
export function DayPlan() {
  ...
  if (activities.length === 0) {
    return <EmptyState title="Dein Plan ist noch leer" description="Du hast für heute noch keine Aktivitäten geplant. Das ist völlig in Ordnung – dein Tag darf flexibel bleiben." />;
  }
  ...
  return (
    <section className="mx-auto flex w-full max-w-md flex-col gap-3" aria-labelledby="plan-heading">
        ...
        {activities.map((activity, index) => (
          <article key={activity.id} className={`px-3 py-2.5 ${index !== activities.length - 1 ? "border-b border-base-300/50" : ""}`}>
            <div className="flex min-h-16 items-center gap-3">
              <span className="flex size-11 shrink-0 items-center justify-center text-2xl leading-none" aria-hidden="true">{activity.type === "quest" ? "🎯" : "🧳"}</span>
      ...
      <button type="button" disabled aria-label="Aktivität hinzufügen" className="min-h-11 w-full rounded-xl bg-primary px-4 text-sm font-semibold text-primary-content shadow-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary disabled:cursor-default">+ Aktivität hinzufügen</button>
