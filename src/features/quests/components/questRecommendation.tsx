  ...
  if (activeQuest?.status === "completed") {
    return (
      <section className="space-y-5" aria-labelledby="quest-complete-heading">
        <header className="min-h-[142px] space-y-2">
        ...
        <article className="card min-h-[190px] border border-primary/20 bg-base-100 shadow-sm">
    ...
    return (
      <section className="space-y-5" aria-labelledby="active-quest-heading">
        <header className="min-h-[142px] space-y-2">
          ...
          <h1 id="active-quest-heading" className="min-h-[58px] text-2xl font-bold tracking-tight">{selectedQuest.title}</h1>
        ...
        <article className="card min-h-[230px] border border-primary/20 bg-base-100 shadow-sm">
          <div className="card-body flex flex-col gap-3">
            <span className="badge badge-primary w-fit">In Arbeit</span>
            <p className="min-h-[52px] text-sm leading-6">{selectedQuest.description}</p>
  ...
  return (
    <section className="space-y-5" aria-labelledby="quest-heading">
      <header className="min-h-[168px] space-y-2">
        ...
        <h1 id="quest-heading" className="min-h-[58px] text-2xl font-bold tracking-tight">Eine passende Aufgabe wartet auf dich.</h1>
        <p className="min-h-[72px] text-sm leading-6 text-base-content/70">Deine Empfehlung orientiert sich an dem Bereich, der gerade am meisten Unterstützung gebrauchen kann.</p>
      </header>
      <article className="card min-h-[265px] border border-primary/20 bg-base-100 shadow-sm">
        <div className="card-body flex flex-col gap-3">
          ...
          <div>
            <h2 className="min-h-[30px] text-xl font-bold">{selectedQuest.title}</h2>
            <p className="mt-2 min-h-[72px] line-clamp-3 text-sm leading-6 text-base-content/70">{selectedQuest.description}</p>
          </div>
          <div className="mt-auto flex items-center justify-between text-sm text-base-content/60">
            <span>Aufwand: {selectedQuest.effort === "short" ? "kurz" : "mittel"}</span>
