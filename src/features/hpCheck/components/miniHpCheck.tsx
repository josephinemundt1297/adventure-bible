    ...
    return (
      <section className="mx-auto flex w-full max-w-md flex-col gap-3" aria-labelledby="mini-hp-recommendation-heading">
        <header className="space-y-1 text-center">
          ...
          <p className="text-xs leading-4 text-base-content/65">Dein Check ist gespeichert. Wähle eine Aufgabe – oder nimm die Alternative.</p>
        </header>
        ...
        <div className="grid gap-2">
          {primary && (
            <article className={`rounded-2xl border p-3 shadow-sm ${selectedQuestId === primary.id ? "border-primary bg-primary/10" : "border-primary/30 bg-primary/5"}`}>
              <div className="flex items-center justify-between gap-2"><span className="badge badge-primary badge-sm">Vorschlag</span><span className="text-xs font-bold text-primary">+{primary.rewardXp} XP</span></div>
              ...
              <button type="button" className="btn btn-primary btn-sm mt-2 min-h-9 w-full" onClick={() => chooseQuest(primary)} aria-pressed={selectedQuestId === primary.id}>{selectedQuestId === primary.id ? "Ausgewählt ✓" : "Diese Aufgabe wählen"}</button>
            </article>
          )}
          {alternative && (
            <article className={`rounded-2xl border p-3 shadow-sm ${selectedQuestId === alternative.id ? "border-primary bg-primary/10" : "border-base-300 bg-base-100"}`}>
              <div className="flex items-center justify-between gap-2"><span className="badge badge-ghost badge-sm">Alternative</span><span className="text-xs font-bold text-base-content/60">+{alternative.rewardXp} XP</span></div>
              ...
              <button type="button" className="btn btn-outline btn-sm mt-2 min-h-9 w-full" onClick={() => chooseQuest(alternative)} aria-pressed={selectedQuestId === alternative.id}>{selectedQuestId === alternative.id ? "Ausgewählt ✓" : "Alternative wählen"}</button>
            </article>
          )}
        </div>
      ...
      <div className="grid gap-2.5" aria-label="Aktuellen Zustand einschätzen">
        {areas.map(({ id, label, icon }) => (
          <div key={id} className="space-y-0.5">
            <div className="flex items-center justify-between gap-2 text-sm"><label htmlFor={`mini-hp-${id}`} className="font-medium"><span aria-hidden="true" className="mr-1.5">{icon}</span>{label}</label><output htmlFor={`mini-hp-${id}`} className="text-xs font-bold text-primary">{values[id]}/100</output></div>
