    ...
    return (
      <section className="mx-auto max-w-md space-y-5" aria-labelledby="hp-result-heading">
        <header className="space-y-2">
          ...
          <p className="text-sm leading-6 text-base-content/70">Deine Einschätzung ist die Grundlage für eine Quest, die zu deinem aktuellen Zustand passt.</p>
        </header>
        <div className="card border border-base-300 bg-base-100 shadow-sm">
          <div className="card-body items-center text-center">
            ...
            <span className="text-5xl font-bold text-primary" aria-label={`${completedState.overall} von 100`}>{completedState.overall}</span>
          ...
          <div className="card-body gap-4">
            ...
            <div className="space-y-4">
              {completedState.areas.map(({ area, score }) => (
                <div key={area}>
                  <div className="mb-1 flex items-center justify-between text-sm"><span>{hpAreaLabels[area]}</span><span className="font-semibold">{score}/100</span></div>
  ...
  return (
    <section className="mx-auto max-w-md space-y-5" aria-labelledby="hp-heading">
      <header className="space-y-2">
        ...
        <p className="text-sm leading-6 text-base-content/70">Nimm dir kurz Zeit, deinen aktuellen Zustand wahrzunehmen. Es gibt keine richtigen oder falschen Antworten.</p>
      </header>
      <div aria-label={`Fortschritt: Frage ${questionIndex + 1} von ${hpQuestions.length}`}>
        <div className="mb-2 flex items-center justify-between text-xs font-semibold"><span>{hpAreaLabels[question.area]}</span><span>Frage {questionIndex + 1} / {hpQuestions.length}</span></div>
      ...
      <fieldset className="card border border-base-300 bg-base-100 shadow-sm">
        <div className="card-body gap-4">
          ...
          <button type="button" className="btn btn-primary mt-2 w-full" disabled={currentAnswer === undefined} onClick={next}>{isLastQuestion ? "Zustand ansehen" : "Weiter"}</button>
        </div>
      </fieldset>
      <p className="text-center text-xs leading-5 text-base-content/60">Dieser Check ist eine persönliche Einschätzung und keine medizinische Diagnose.</p>
    </section>
